...
_MAX_MEMORY_CONTENT_LEN = int(os.getenv("MAX_MEMORY_CONTENT_LEN", "16000"))
...
async def delete_all_memories(user_id: str, request: Request, engine: Optional[AsyncEngine] = Depends(get_engine_dep)):
    ...
    async with engine.begin() as conn:
        res = await conn.execute(
            t_memories.delete().where(t_memories.c.user_id == user_id)
        )
...
async def export_memories(user_id: str, request: Request, engine: Optional[AsyncEngine] = Depends(get_engine_dep)):
    ...
    if engine is not None:
        async with engine.connect() as conn:
            res = await conn.execute(
                select(t_memories)
                .where(t_memories.c.user_id == user_id)
                .order_by(t_memories.c.created_at.asc())
            )
    ...
    async with engine.begin() as conn:
        res = await conn.execute(
            t_memories.delete().where(
                (t_memories.c.id == memory_id) & (t_memories.c.user_id == user_id)
            )
        )
        if (res.rowcount if hasattr(res, "rowcount") else 0) == 0:
            raise HTTPException(status_code=404, detail="Memory not found")
...
async def get_memory_sync_status(user_id: str, request: Request, engine: Optional[AsyncEngine] = Depends(get_engine_dep)):
    ...
    if engine is not None:
        async with engine.connect() as conn:
            res = await conn.execute(
                select(func.count(t_memories.c.id)).where(t_memories.c.user_id == user_id)
            )
            row = res.first()
...
async def full_memory_sync(user_id: str, request: Request, engine: Optional[AsyncEngine] = Depends(get_engine_dep)):
    ...
    status = await get_memory_sync_status(user_id, request, engine)
    return {
        "userId": user_id,
