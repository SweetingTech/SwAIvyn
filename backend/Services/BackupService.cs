...
namespace SwAIvyn.Services
{
    ...
    public class BackupService : BackgroundService
    {
        ...
        private async Task CreateBackupAsync()
        {
            ...
            try
            {
                ...
                using (var zipArchive = ZipFile.Open(backupPath, ZipArchiveMode.Create))
                {
                    ...
                    var neo4jDataPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "SwAIvyn", "neo4j", "data");
                    if (Directory.Exists(neo4jDataPath))
                    {
                        _logger.LogInfo($"Adding Neo4j data to backup: {neo4jDataPath}");
                        AddDirectoryToZip(zipArchive, neo4jDataPath, "neo4j/data");
                    }

                    // Add Neo4j logs if available
                    var neo4jLogsPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "SwAIvyn", "neo4j", "logs");
                    if (Directory.Exists(neo4jLogsPath))
                    {
                        _logger.LogInfo($"Adding Neo4j logs to backup: {neo4jLogsPath}");
                        AddDirectoryToZip(zipArchive, neo4jLogsPath, "neo4j/logs");
                    }
                    ...
                    using (var writer = new StreamWriter(metadataEntry.Open()))
                    {
                        ...
                        await writer.WriteLineAsync($"Neo4j data included: {Directory.Exists(neo4jDataPath)}");
                    }
                }
        ...
        /// <param name="entryPrefix">Prefix for entries in the ZIP archive</param>
        private void AddDirectoryToZip(ZipArchive archive, string directoryPath, string entryPrefix)
        {
            try
            {
                ...
                {
                    // Skip large log files and temporary files
                    if (file.EndsWith(".log") && new FileInfo(file).Length > 10 * 1024 * 1024)
                    {
                        _logger.LogInfo($"Skipping large log file: {file}");
                        continue;
                    }
                    ...
                    {
                        // File might be in use, log and continue
                        _logger.LogWarning($"Could not add file to backup: {file}. Error: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error adding directory to backup: {directoryPath}", ex);
            }
        }
    }
}
