    ...
    public class Neo4jRuntimeService : IDisposable
    {
        ...
        public async Task InitializeAsync()
        {
            try
            {
                ...
                // Check if Neo4j is already extracted and has required binaries
                if (!Directory.Exists(_neo4jHomePath) || !File.Exists(Path.Combine(_neo4jBinPath, RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "neo4j.bat" : "neo4j")))
                {
                    await ExtractNeo4jAsync();
                }
                else
                {
                    _logger.LogInfo("Neo4j already extracted, skipping extraction");
                }
                ...
                _logger.LogInfo("Neo4j runtime initialized successfully");
            }
            catch (Exception ex)
            {
                ...
                if (!File.Exists(neo4jZipPath))
                {
                    ...
                    if (!File.Exists(neo4jZipPath))
                    {
                        ...
                        if (Directory.Exists(directory))
                        {
                            ...
                            {
                                _logger.LogWarning("No Neo4j ZIP file found, but Neo4j directory exists. Assuming Neo4j is already extracted.");
                                return;
                            }
                        }
                        else
        ...
        private void UpdateNeo4jConfiguration()
        {
            try
            {
                ...
                {
                    throw new FileNotFoundException($"Neo4j configuration file not found at {_neo4jConfPath}");
                }
