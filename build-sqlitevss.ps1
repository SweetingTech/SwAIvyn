...
$sqliteVssDir = Get-Location

# Patch root CMakeLists.txt
(Get-Content "$sqliteVssDir\CMakeLists.txt") | ForEach-Object {
    $_ -replace 'cmake_minimum_required(VERSION 3)', 'cmake_minimum_required(VERSION 3.5)'
} | Set-Content "$sqliteVssDir\CMakeLists.txt"

# Patch vendor/faiss/CMakeLists.txt
(Get-Content "$sqliteVssDir\vendor\faiss\CMakeLists.txt") | ForEach-Object {
    $_ -replace 'cmake_minimum_required(VERSION 3)', 'cmake_minimum_required(VERSION 3.5)'
} | Set-Content "$sqliteVssDir\vendor\faiss\CMakeLists.txt"

# Patch vendor/json/CMakeLists.txt
(Get-Content "$sqliteVssDir\vendor\json\CMakeLists.txt") | ForEach-Object {
    $_ -replace 'cmake_minimum_required(VERSION 3)', 'cmake_minimum_required(VERSION 3.5)'
} | Set-Content "$sqliteVssDir\vendor\json\CMakeLists.txt"
...
if (-not (Test-Path $SQLITE_TARBALL)) {
    ...
    # Create vendor/sqlite directory if it doesn't exist
    $vendorDir = Join-Path (Get-Location) "vendor"
    $sqliteVendorDir = Join-Path $vendorDir "sqlite"
    if (-not (Test-Path $vendorDir)) {
        New-Item -ItemType Directory -Path $vendorDir | Out-Null
    }
    if (-not (Test-Path $sqliteVendorDir)) {
        New-Item -ItemType Directory -Path $sqliteVendorDir | Out-Null
    }

    # Extract the tarball directly into vendor/sqlite
    Write-Host "Extracting SQLite source directly into vendor/sqlite..."
    tar -xzf $SQLITE_TARBALL -C $sqliteVendorDir
    Rename-Item -Path (Join-Path $sqliteVendorDir "sqlite-autoconf-$SQLITE_VERSION_NUMBER") -NewName (Join-Path $sqliteVendorDir "sqlite-source") -Force
    Write-Host "After rename, sqlite-source exists: $(Test-Path 'sqlite-source')"

    # Move sqlite-source to vendor/sqlite as expected by CMakeLists.txt
    Write-Host "Current Location: $(Get-Location)"
    $vendorDir = Join-Path (Get-Location) "vendor"
    $sqliteVendorDir = Join-Path $vendorDir "sqlite"
    Write-Host "Vendor Dir: $vendorDir"
    Write-Host "SQLite Vendor Dir: $sqliteVendorDir"
    $vendorDir = Join-Path (Get-Location) "vendor"
    $sqliteVendorDir = Join-Path $vendorDir "sqlite"
    if (-not (Test-Path $vendorDir)) {
        New-Item -ItemType Directory -Path $vendorDir | Out-Null
    ...
    Move-Item -Path (Join-Path (Get-Location) "sqlite-source") -Destination $sqliteVendorDir -Force
    Write-Host "Move-Item successful: $(Test-Path $sqliteVendorDir\sqlite-source)"
}
