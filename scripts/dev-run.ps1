...
function Invoke-GracefulShutdown {
  ...
  $shutdownScript = $null
  try {
    $shutdownScript = Resolve-DevScript 'dev-shutdown.ps1'
  } catch {
    $shutdownScript = $null
  }

  if (-not $shutdownScript -or -not (Test-Path $shutdownScript)) {
...
function Start-ServiceScript {
  ...
  if (-not (Test-Path $ScriptPath)) {
    Write-Warning "Script for $ServiceName not found at $ScriptPath"
    return $null
  }
...
function Resolve-DevScript {
  param([Parameter(Mandatory = $true)][string]$Name)

  $candidates = @(
    Join-Path $PSScriptRoot $Name,
    Join-Path (Join-Path $PSScriptRoot 'old-scripts') $Name
  )
  ...
  throw "Required development script not found: $Name"
}
...
$setupScript = $null
try {
  $setupScript = Resolve-DevScript 'setup.ps1'
} catch {
  $setupScript = $null
}
...
if (-not $BackendOnly) {
    # --- Start Frontend ---
    $frontendScript = Resolve-DevScript 'dev-frontend.ps1'
...
if (-not $FrontendOnly) {
    ...
    if ($pgReady) {
        $bffScript = Resolve-DevScript 'dev-bff.ps1'
    ...
    # --- Start Orchestrator ---
    if ($temporalReady) {
        $orchestratorScript = Resolve-DevScript 'dev-orchestrator.ps1'
...
if ($temporalReady) {
    ...
    try {
        $describeArgs = @(
            'run','--rm','--network',"${StackName}_default",
            'temporalio/admin-tools:1.23','temporal','operator','namespace','describe',
            '--namespace','default','--address','temporal:7233'
        )
        $null = & docker @describeArgs 2>$null

        if ($LASTEXITCODE -ne 0) {
            $createArgs = @(
                'run','--rm','--network',"${StackName}_default",
                'temporalio/admin-tools:1.23','temporal','operator','namespace','create',
                '--namespace','default','--address','temporal:7233','--retention','3d','--yes'
            )
...
# --- FINALIZATION ---
$dividerLine = [string]::new('=', 60)
Write-Host ("`n" + $dividerLine) -ForegroundColor Yellow
Write-Host "🚀 SwAIvyn Development Environment Ready!" -ForegroundColor Green
Write-Host $dividerLine -ForegroundColor Yellow
...
$startupSucceeded = $true
    ...
    Write-Log -Message ("Startup failed: {0}" -f $errorMessage) -Level 'ERROR'
    Invoke-GracefulShutdown -Reason $errorMessage -Stack $StackName
}
finally {
    if ($startupSucceeded) {
        Write-Log -Message ("SwAIvyn startup completed successfully. Log file: {0}" -f $script:LogFile) -Level 'SUCCESS'
    } else {
        Write-Log -Message ("SwAIvyn startup failed. Review log file: {0}" -f $script:LogFile) -Level 'ERROR'
    }
