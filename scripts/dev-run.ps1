...
function Invoke-GracefulShutdown {
  ...
  $shutdownScript = Join-Path $scriptsDir 'dev-shutdown.ps1'
...
if (-not $BackendOnly) {
    # --- Start Frontend ---
    $frontendScript = Join-Path $rootDir 'scripts/dev-frontend.ps1'
...
if (-not $FrontendOnly) {
    ...
    if ($pgReady) {
        $bffScript = Join-Path $rootDir 'scripts/dev-bff.ps1'
    ...
    # --- Start Orchestrator ---
    if ($temporalReady) {
        $orchestratorScript = Join-Path $rootDir 'scripts/dev-orchestrator.ps1'
