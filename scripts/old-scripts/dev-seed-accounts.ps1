# Force (re)seed core accounts into the Postgres DB used by the BFF.
# - Creates/uses the BFF venv
# - Loads .env for POSTGRES_PASSWORD
# - Builds DATABASE_URL if missing
# - Runs app.seed_reset_accounts to upsert admin/mari/djay and default character
...
# 2) Safety prompt
if (-not $Yes) {
  $resp = Read-Host 'This will reset passwords for admin/mari/djay and upsert a default character. Continue? (y/N)'
...
# 4) Build DATABASE_URL if missing from POSTGRES_PASSWORD (matches dev-bff.ps1)
...
if (-not $env:DATABASE_URL) {
  Write-Warning 'DATABASE_URL is not set and POSTGRES_PASSWORD not provided. Using whatever is configured in the environment.'
}

# 5) Run the force seeder
Write-Host 'Seeding accounts (admin/mari/djay) and default character...' -ForegroundColor Green
python -m app.seed_reset_accounts
