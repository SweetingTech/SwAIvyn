# Test script for SwAIvyn development environment
param (
...
if (-not $SkipServiceTests) {
    $serviceScripts = @{
        'dev-bff.ps1' = @{ Path = 'scripts/dev-bff.ps1'; Args = @('-Help') }
        'dev-frontend.ps1' = @{ Path = 'scripts/dev-frontend.ps1'; Args = @() }
        'dev-orchestrator.ps1' = @{ Path = 'scripts/dev-orchestrator.ps1'; Args = @() }
    }
    
    foreach ($script in $serviceScripts.Keys) {
        $scriptInfo = $serviceScripts[$script]
        $scriptPath = Join-Path $rootDir $scriptInfo.Path
