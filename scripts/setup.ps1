...
[CmdletBinding()]
param(
    # Retain Mode to keep existing callers compatible, but it won't affect DB-only run
    [ValidateSet('Development', 'Production', 'Service')]
    [string]$Mode = 'Production',

    # DB-only options
    [switch]$CleanInstall,
    [string]$DatabasePath = "Sqldatabase\swai-vyn.db",

    # Which stores to initialize (all on by default)
    [switch]$InitSqlite = $true,
    [switch]$InitWeaviate = $true,
    [switch]$InitNeo4j = $true,

    # External services
    [string]$WeaviateUrl = 'http://localhost:8080',
    [string]$Neo4jUri = 'http://localhost:7474',
    [string]$Neo4jUser = 'neo4j',
    [string]$Neo4jPassword = 'test12345',

    # Vector config
    [int]$VectorDimensions = 768,

    # Ensure we prune to DB-only behavior
    [switch]$DatabasesOnly = $true
)
...
# Databases-only helpers and flow
function New-BasicAuthHeader($User,$Password){
    $pair   = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes("$User`:$Password"))
    return @{ Authorization = "Basic $pair"; 'Content-Type' = 'application/json' }
}

function Initialize-Weaviate {
    param([string]$BaseUrl,[int]$VectorDimensions)
    Write-Host "`n[Weaviate] Initialization" -ForegroundColor Magenta
    try {
        $meta = Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/v1/meta" -TimeoutSec 10
        if ($meta.StatusCode -ne 200) { Write-Warning "Weaviate meta not OK: $($meta.StatusCode)"; return $false }
        $schemaResp = Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/v1/schema" -TimeoutSec 10
        $schemaJson = $schemaResp.Content
        $needMemory   = -not ($schemaJson -match 'SwAIvynMemory')
        $needDocument = -not ($schemaJson -match '"Document"')

        if ($needMemory) {
            Write-Info "Creating Weaviate class SwAIvynMemory"
            $memoryClass = @{ class='SwAIvynMemory'; description='SwAIvyn memory storage for vector search'; vectorizer='none'; vectorIndexConfig=@{ distance='cosine' }; properties=@(
                @{ name='content';   dataType=@('text');   description='The memory content' },
                @{ name='category';  dataType=@('string'); description='Memory category' },
                @{ name='userId';    dataType=@('string'); description='User ID who owns this memory' },
            ...
            $body = ($memoryClass | ConvertTo-Json -Depth 6)
            Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/v1/schema" -Method Post -ContentType 'application/json' -Body $body | Out-Null
            Write-Success "Created Weaviate class SwAIvynMemory"
        } else { Write-Info "Weaviate class SwAIvynMemory already exists" }

        if ($needDocument) {
            Write-Info "Creating Weaviate class Document"
            $docClass = @{ class='Document'; description='Document chunks from uploaded files for semantic search'; vectorizer='none'; vectorIndexConfig=@{ distance='cosine' }; properties=@(
                @{ name='title';      dataType=@('text');   description='Document title or filename' },
                @{ name='content';    dataType=@('text');   description='Document chunk content' },
                @{ name='source';     dataType=@('string'); description='Source filename or URL' },
                @{ name='tags';       dataType=@('text[]'); description='Document tags for categorization' },
                @{ name='userId';     dataType=@('string'); description='User ID who owns this document' },
                @{ name='uploadedAt'; dataType=@('date');   description='Upload timestamp' },
                @{ name='chunkIndex'; dataType=@('int');    description='Index of this chunk within the document' },
                @{ name='metadata';   dataType=@('text');   description='Additional metadata as JSON' }
            ) }
            $body = ($docClass | ConvertTo-Json -Depth 6)
            Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/v1/schema" -Method Post -ContentType 'application/json' -Body $body | Out-Null
            Write-Success "Created Weaviate class Document"
        } else { Write-Info "Weaviate class Document already exists" }
        return $true
    } catch { Write-Error "Weaviate init error: $($_.Exception.Message)"; return $false }
}

function Initialize-Neo4j {
    param([string]$Uri,[string]$User,[string]$Password,[int]$VectorDimensions)
    Write-Host "`n[Neo4j] Initialization" -ForegroundColor Magenta
    try {
        $headers = New-BasicAuthHeader $User $Password
        $commit  = "$Uri/db/neo4j/tx/commit"
        $okBody  = @{ statements=@(@{ statement='RETURN 1 AS ok'; parameters=@{}; includeStats=$true }) } | ConvertTo-Json -Depth 5
        $okResp  = Invoke-WebRequest -UseBasicParsing -Uri $commit -Method Post -Headers $headers -Body $okBody -TimeoutSec 10
        if ($okResp.StatusCode -lt 200 -or $okResp.StatusCode -ge 300) { Write-Warning "Neo4j ping bad status: $($okResp.StatusCode)"; return $false }
        $queries = @(
            'CREATE CONSTRAINT memory_id_unique IF NOT EXISTS FOR (m:Memory) REQUIRE m.id IS UNIQUE',
            "CREATE VECTOR INDEX memory_embedding_vector IF NOT EXISTS FOR (m:Memory) ON (m.embedding) OPTIONS {indexConfig: {`vector.dimensions`: $VectorDimensions, `vector.similarity_function`: 'cosine'}}",
        ...
        foreach ($q in $queries) {
            $body = @{ statements=@(@{ statement=$q; parameters=@{}; includeStats=$true }) } | ConvertTo-Json -Depth 5
            try {
                $r = Invoke-WebRequest -UseBasicParsing -Uri $commit -Method Post -Headers $headers -Body $body -TimeoutSec 20
                Write-Info "Applied Neo4j DDL: $q"
            } catch { Write-Warning "Neo4j DDL failed (likely exists): $q -> $($_.Exception.Message)" }
        }
        Write-Success "Neo4j schema ensured"
        return $true
    } catch { Write-Error "Neo4j init error: $($_.Exception.Message)"; return $false }
}


if ($DatabasesOnly) {
    Write-Host "`n[DB] DATABASES-ONLY MODE" -ForegroundColor Magenta

    # Determine project root and normalize DB path
    $projectRoot = Split-Path -Parent $PSScriptRoot
    if (-not ([System.IO.Path]::IsPathRooted($DatabasePath))) { $DatabasePath = Join-Path $projectRoot $DatabasePath }

    # Clean SQLite files if requested
    if ($CleanInstall) {
        Write-Info "CleanInstall: removing $DatabasePath (-wal/-shm)"
        foreach ($f in @($DatabasePath, "$DatabasePath-wal", "$DatabasePath-shm")) { if (Test-Path $f) { Remove-Item $f -Force -ErrorAction SilentlyContinue } }
    }

    # Ensure DB directory exists
    $dbParentDir = Split-Path $DatabasePath -Parent
    if ($dbParentDir -and -not (Test-Path $dbParentDir)) { New-Item -ItemType Directory -Path $dbParentDir -Force | Out-Null }

    $results = @{ sqlite=$false; weaviate=$false; neo4j=$false }

    if ($InitSqlite) {
        Write-Host "`n[SQLite] Schema + Seed" -ForegroundColor Magenta
        $toolsToBuild = @('CreateTables','UpdateDatabase')
        foreach ($toolName in $toolsToBuild) {
            $toolDir = Join-Path $projectRoot "tools/$toolName"
            if (Test-Path $toolDir) {
                Push-Location $toolDir
                dotnet build -c Release | Out-Null
                $exe = Join-Path $toolDir "bin/Release/net8.0/$toolName.exe"
                if (Test-Path $exe) { & $exe | Out-Null } else { Write-Warning "Tool executable not found: $exe" }
                Pop-Location
            } else { Write-Warning "Tool directory not found: $toolDir" }
        }
        $results.sqlite = Test-Path $DatabasePath
    }

    if ($InitWeaviate) { $results.weaviate = Initialize-Weaviate -BaseUrl $WeaviateUrl -VectorDimensions $VectorDimensions }
    if ($InitNeo4j)   { $results.neo4j    = Initialize-Neo4j   -Uri $Neo4jUri -User $Neo4jUser -Password $Neo4jPassword -VectorDimensions $VectorDimensions }

    Write-Host "`n===== DATABASES SUMMARY =====" -ForegroundColor Cyan
    Write-Host ("SQLite : {0}" -f ($(if($results.sqlite){'ready'}else{'not-ready'})))
    Write-Host ("Weaviate: {0}" -f ($(if($results.weaviate){'ready'}else{'not-ready'})))
    Write-Host ("Neo4j   : {0}" -f ($(if($results.neo4j){'ready'}else{'not-ready'})))
    Write-Host "==============================`n" -ForegroundColor Cyan
    return
}

<# DB-ONLY: ignore the remainder of this file. The database-only flow above already returned. #>

<#
Write-Info "Setup Mode: $Mode"
Write-Info "DatabasesOnly: $DatabasesOnly"
Write-Info "Database Path: $DatabasePath"
Write-Info "WeaviateUrl: $WeaviateUrl"
Write-Info "Neo4jUri: $Neo4jUri"
Write-Info "VectorDimensions: $VectorDimensions"
...
# Database file validation (if not CleanInstall)
...
# Create missing schema and seed default data via tools
Write-Info "Ensuring database schema & defaults using tools/CreateTables and tools/UpdateDatabase..."
$toolsToBuild = @('CreateTables', 'UpdateDatabase')
foreach ($toolName in $toolsToBuild) {
    $toolDir = Join-Path $projectRoot "tools\$toolName"
    if (Test-Path $toolDir) {
...
Write-Success "Database schema and default data ensured."
    ...
    if ($settingsContent) {
        ...
        try {
            ...
            Write-Success "Successfully updated $appsettingsDistPath."
             ...
             Write-Error ("Failed to write updated {0}: {1}" -f $appsettingsDistPath, $_.Exception.Message)
        }
    } else {
