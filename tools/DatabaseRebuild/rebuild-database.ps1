...
try {
    ...
    foreach ($statement in $statements) {
        ...
        if ($trimmedStatement -ne '' -and -not $trimmedStatement.StartsWith('--')) {
            try {
                ...
                Write-Host "⚠️  Warning: Failed to execute statement: $($_.Exception.Message)" -ForegroundColor Yellow
    ...
    $connection.Close()
    Write-Host "`n🎉 Database rebuild completed successfully!" -ForegroundColor Green
