...
{
    /// <summary>
    /// Script to insert GLaDOS character into the database from YAML file
    /// </summary>
    public class InsertGLaDOSCharacter
    {
        public static async Task Main(string[] args)
        {
            Console.WriteLine("🚀 Starting GLaDOS Character Insertion Script...");
            ...
            try
            {
                ...
                var existingGLaDOS = await dbContext.Avatars.FirstOrDefaultAsync(a => a.Name == "GLaDOS");
                if (existingGLaDOS != null)
                {
                    Console.WriteLine("⚠️ GLaDOS character already exists in database!");
                    Console.WriteLine($"   ID: {existingGLaDOS.Id}");
                    Console.WriteLine($"   SystemPrompt Length: {existingGLaDOS.SystemPrompt?.Length ?? 0}");
                    Console.WriteLine("   Skipping insertion.");
                    return;
                }

                // Read GLaDOS YAML file
                string yamlPath = Path.Combine("frontend", "AI", "GLaDOS", "GLaDOS_Character_card.yaml");
                if (!File.Exists(yamlPath))
                {
                    Console.WriteLine($"❌ GLaDOS YAML file not found at: {yamlPath}");
                    return;
                }

                Console.WriteLine($"📄 Reading GLaDOS YAML from: {yamlPath}");
                ...
                // Create AvatarInfo entity
                var gladosAvatar = new AvatarInfo
                {
                    Id = Guid.NewGuid(),
                    UserId = defaultUserId,
                    Name = yaml["name"]?.ToString() ?? "GLaDOS",
                ...
                Console.WriteLine("💾 Inserting GLaDOS into database...");
                dbContext.Avatars.Add(gladosAvatar);
                await dbContext.SaveChangesAsync();

                Console.WriteLine("✅ GLaDOS character inserted successfully!");
                Console.WriteLine($"   ID: {gladosAvatar.Id}");
                Console.WriteLine($"   SystemPrompt Preview: {systemPrompt.Substring(0, Math.Min(100, systemPrompt.Length))}...");
                Console.WriteLine("");
                Console.WriteLine("🎉 GLaDOS is now ready for testing!");
                Console.WriteLine("   Try sending a message to any conversation and GLaDOS should respond.");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"❌ Error inserting GLaDOS: {ex.Message}");
...
".Trim();
            ...
            {
                return "You are GLaDOS, the sarcastic AI from Aperture Science.";
            }
        }
    }
}
